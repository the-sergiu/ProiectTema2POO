...
bool Utility::compareProdusAlimentar(Product *p1, Product *p2)
{
    ...
    if (p1_pa->getName() < p2->getName()) return true;
    if (p1_pa->getName() > p2->getName()) return false;

    if (p1_pa->getCountryOfOrigin() < p2_pa->getCountryOfOrigin()) return true;
    if (p1_pa->getCountryOfOrigin() > p2_pa->getCountryOfOrigin()) return false;
